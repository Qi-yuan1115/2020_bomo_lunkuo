...
//复原波形
double Bomo_paint_X_RALL[Bomo_paint_arrayLength];
double Bomo_paint_Y_TeeChartArray_RALL[Bomo_paint_arrayLength];
...
UINT CHighPrecisionDlg::Bomo_Shujun_Caiji_01(LPVOID pParam)
{
	...
	while (!Bomo_Flag_bomo_Caiji_R)	// 10 cycles
	{
		...
		{
			fprintf(fpz11, " %0.3fm ,   %f \n", caiji_length*Bomo_Caiyang_jiange, data[j]);
			caiji_length++;
		}
...
void CHighPrecisionDlg::Bomo_Huatu_DrawMoving_Calculate_three()
{
	...
	double *lineALL = new double[Bomo_huatu_yidong_length];
	//添加三条线
	...
	//
	//Bomo_pLineSerie_RALL = Bomo_ChartCtrl_R2.CreateLineSerie();

	////Bomo_pLineSerie_R3->SetColor(RGB( 0,0,255));
	//Bomo_pLineSerie_RALL->SetSeriesOrdering(poNoOrdering);//设置为无序  
	...
	/*				Bomo_pLineSerie_RALL->AddPoints(Bomo_paint_X_R3, Bomo_paint_Y_TeeChartArray_R3, Bomo_paint_arrayLength);*/
	...
	//正常的处理阶段
			else
			{
				...
				if (Bomo_paint_step_R < Bomo_length)//画图
				{
					...
					/*	lineALL[m] = Bomo_a_10_100Hz_signal_R[Bomo_paint_step_R];*/
						Bomo_paint_step_R++;
					}
					///////////////////画图//////////////////////////
					...
					//Bomo_Huatu_LeftMoveArray(Bomo_paint_Y_TeeChartArray_RALL, Bomo_paint_arrayLength, line1);//移动y坐标
					//Bomo_Huatu_LeftMoveArray_shuju(Bomo_paint_X_RALL, Bomo_paint_arrayLength, Bomo_paint_X_cout_R);//移动x坐标
...
void CHighPrecisionDlg::Bomo_Huatu_LeftMoveArray_shuju(double * ptr, size_t length2, double data)
{
	...
	for (size_t i = 0; i<length2; ++i)
		...
		ptr[i] = (data - Bomo_paint_arrayLength + i)*0.002;
}
//刷新函数
